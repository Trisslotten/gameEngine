...
public class Player extends Entity implements Serializable {
	...
	public void update(double dt, Game game) {
		if (xpos != game.getTx() && ypos != game.getTy() && game.getTx() != 0 && game.getTy() != 0) {
			double dx = windowWidth / 2 - game.getTx();
			double dy = windowHeight / 2 - game.getTy();
			double angle = Math.atan2(dy, dx);
			xspd = Math.cos(angle) * velocity * dt / 1000;
			yspd = Math.sin(angle) * velocity * dt / 1000;
		}
		/*
		 * double dx = windowWidth/2-game.cursor.getXpos(); double dy =
		 * windowHeight/2-game.cursor.getYpos(); double angle = Math.atan2(dy,
		 * dx); xspd = Math.cos(angle)*velocity*dt/1000; yspd =
		 * Math.sin(angle)*velocity*dt/1000;
		 */
